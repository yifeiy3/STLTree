...
def handlerThings() {
    ...
    things.each {
        ...
        def attri = []
        def attrivalues = []
        it.getSupportedAttributes().each {
        	attri << it.name
            attrivalues << it.values
        }
        log.debug(attri)
...
def handlerStates(){
    ...
    def max = params.max ? params.max.toInteger() : 50000
    max = 0 < max && max < 50000 ? max : 50000
